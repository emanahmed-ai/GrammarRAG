...
BRAND_NAME = "Verity AI"
BRAND_SUBTITLE = "INTELLIGENT ENGLISH ASSISTANT"
BRAND_TAGLINE = "Intelligent English."
BRAND_TAGLINE_ACCENT = "Clearer Expression."
BRAND_SUBTAGLINE = "AI-powered grammar correction & learning"
...
def initialize_theme() -> None:
    """Ensure theme-related and app session state keys exist before any
    widget touches them."""
...
def _clear_analysis() -> None:
    ...
    `analysis_history`, `last_history_id`, `current_examples`, `theme`,
    or `current_page` - only the current-attempt state is cleared.
    """
    st.session_state.sentence_input = ""
...
def _toggle_theme() -> None:
    """Callback: flip between the dark (default) and light palette."""
...
def inject_custom_css() -> None:
    ...
    if st.session_state.theme == "light":
        ...
        --panel-light: rgba(20, 24, 38, 0.07);
        --panel-border: rgba(120, 100, 40, 0.25);
        --ivory: #1c1a14;
        --muted-ivory: #4b4738;
        --gold: #7a5710;
        --gold-light: #8a6415;
        ...
        --panel-light: rgba(255, 255, 255, 0.065);
    ...
    st.markdown(
        ...
        html, body, [data-testid="stAppViewContainer"] {{
            {(
                f'''background-image:
                    linear-gradient({"rgba(6,9,17,0.74), rgba(6,9,17,0.88)" if st.session_state.theme == "dark" else "rgba(234,228,211,0.86), rgba(234,228,211,0.93)"}),
                    radial-gradient(circle at 15% 0%, rgba(212,175,55,0.06), transparent 40%),
                    radial-gradient(circle at 85% 10%, rgba(79,107,239,0.08), transparent 45%),
                ...
                f'''background-image:
                    radial-gradient(circle at 15% 0%, rgba(212,175,55,0.06), transparent 40%),
                    radial-gradient(circle at 85% 10%, rgba(79,107,239,0.08), transparent 45%) !important;
                background-color: var(--navy);'''
            )}
        ...
        @media (max-width: 640px) {{
            html, body, [data-testid="stAppViewContainer"] {{
                background-position: {"center, center, center, top center" if _bg_data_uri else "center, center"};
            }}
        }}
        ...
        * {{
        ...
        .lex-nav-item.active {{
            background: linear-gradient(90deg, rgba(79,107,239,0.22), rgba(79,107,239,0.05));
            border-color: rgba(79,107,239,0.45);
            color: var(--ivory);
            box-shadow: inset 0 0 12px rgba(79,107,239,0.15);
        }}
        .lex-nav-icon {{
            width: 18px;
            text-align: center;
        ...
        section[data-testid="stSidebar"] .stButton > button[kind="primary"] {{
            background: linear-gradient(90deg, rgba(79,107,239,0.25), rgba(157,123,240,0.12)) !important;
            border: 1px solid rgba(79,107,239,0.45) !important;
            color: var(--ivory) !important;
            font-weight: 700 !important;
            box-shadow: inset 0 0 12px rgba(79,107,239,0.15) !important;
        }}
        ...
        /* Streamlit text_area override - a dedicated light writing
           ...
           colors (not the theme's --ivory var) so typed text is always
           dark navy on a light background, in both the dark and light
           app themes, and never inherits the gold/ivory body text
           color. Multiple selectors target Streamlit's actual textarea
           DOM across versions. */
        textarea,
        ...
        [data-baseweb="textarea"] textarea::selection {{
            background: rgba(79, 107, 239, 0.28) !important;
            color: #0b1324 !important;
        }}
        .stTextArea textarea:focus {{
            border-color: var(--gold) !important;
            box-shadow: 0 0 0 2px rgba(212,175,55,0.28), 0 0 18px rgba(79,107,239,0.2) !important;
        ...
        .stButton > button[kind="primary"] {{
            background: linear-gradient(90deg, var(--royal-blue), var(--purple)) !important;
            border: none !important;
            color: var(--gold-light) !important;
            font-weight: 700 !important;
            letter-spacing: 0.5px;
            padding: 0.7rem 1rem !important;
            box-shadow: 0 6px 20px rgba(79,107,239,0.35);
        }}
        .stButton > button[kind="primary"]:hover {{
            box-shadow: 0 8px 26px rgba(79,107,239,0.5);
        ...
        .lex-feature-card.f1 {{ border-top: 3px solid var(--royal-blue); }}
        .lex-feature-card.f2 {{ border-top: 3px solid var(--purple); }}
        .lex-feature-card.f3 {{ border-top: 3px solid var(--success); }}
        .lex-feature-card.f4 {{ border-top: 3px solid var(--gold); }}
        ...
        st.markdown(
            ...
            <div class="lex-sidebar-brand">
                ...
                <div class="brand-text">
                    <div class="name">VERITY <span class="ai">AI</span></div>
...
def render_header() -> None:
    """Top-right header controls: theme toggle, notification icon, author
    card."""
    spacer, toggle_col, bell_col, author_col = st.columns([5, 1.4, 0.6, 2])
    with toggle_col:
        label = "\u2600 Light" if st.session_state.theme == "dark" else "\u263D Dark"
        st.button(label, key="theme_toggle_btn", on_click=_toggle_theme, use_container_width=True)
    with bell_col:
        st.markdown('<div class="lex-bell">&#128276;</div>', unsafe_allow_html=True)
    with author_col:
        names = "<br>".join(AUTHORS)
        st.markdown(
            f"""
            <div class="lex-author-card">
...
def render_hero() -> None:
    ...
    st.markdown(
        f"""
        <div class="lex-hero">
            <h1>{BRAND_TAGLINE} <span class="refined">{BRAND_TAGLINE_ACCENT}</span></h1>
            <div class="subtag">{BRAND_SUBTAGLINE}</div>
            <div class="workflow">
                Detect <span class="sep">&#10022;</span> Correct
                <span class="sep">&#10022;</span> Explain
                <span class="sep">&#10022;</span> Learn
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_footer() -> None:
    """Premium footer with copyright and author credit."""
    st.markdown(
        f"""
        <div class="lex-footer">
            &copy; 2025 Verity AI. All rights reserved.
            <div class="crafted">Crafted with Intelligence &middot;
            {" &middot; ".join(AUTHORS)}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_home_page() -> None:
    ...
    st.markdown(
        ...
        <div class="lex-panel" style="text-align:center;">
            <div style="color: var(--muted-ivory); font-size:1.02rem; line-height:1.6; max-width:760px; margin:0 auto;">
                Verity AI pairs semantic retrieval of real grammar-correction
                examples with an AI language model to detect, correct, and
                explain English grammar and syntax errors - so every
